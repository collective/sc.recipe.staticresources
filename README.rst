...
Library generation choices
--------------------------
The default configuration generated by the Javascript Helper has some choices in configuration that should be better explained:

libraryTarget var
^^^^^^^^^^^^^^^^^
Plone 5 uses RequireJS to create module configuration, and the way it is generated `conflict with Webpack UMD and AMD libraries <https://github.com/collective/collective.lazysizes/issues/67>`_.  To avoid this conflict today it is used this configuration to create a simple variable with the library in the end.  Note that this soluction is temporary and should be better addressed in the future.
