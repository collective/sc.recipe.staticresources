...
This recipe has included these features:

1. Yarn
   This is a installation tool better than NPM because it speed up the download of packages by caching it.

2. HardSourceWebpackPlugin
   This is another tool designed to speed up module build.

3. SpritesmithPlugin
   This plugin is used to automatically create sprites combining the images inserted into an specific folder.

4. Git hash
   Our recipe always add a hash generated by current commit revision in the end of the static files.
   The problem solved here is to deal with cache servers updates.
